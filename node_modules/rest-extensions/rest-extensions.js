...
const DIR = path.resolve(__dirname).replace('node_modules/rest-extensions', '') + EXTENSIONS_PATH;
class Trigger {
...
class ExtensionManager {
    ...
    getConfig(name) {
        const configPath = DIR + "/"+ name + '/config.yml';
    ...
    getAvaiableTriggers(resource, method) {
        ...
        const triggersInfo = triggers.filter( item => {
            if (item.resource.indexOf(resource) > -1) {
                if (item.method && item.method.indexOf(method) > -1)
                    return item;
            }
        })
