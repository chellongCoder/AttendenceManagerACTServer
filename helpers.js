...
class Helpers {
    ...
    constructor() {
        ...
        this.sheetsService = client.then((auth) => googleapis.sheets({
            version: 'v4',
            auth
        }));
        this.driveService = client.then((auth) => googleapis.drive({
            version: 'v3',
            auth
        }));
        this.filesToDelete = [];
    ...
    cleanup() {
        return this.driveService.then((drive) => {
            this.filesToDelete.map((fileId) => drive.files.delete({
                fileId
            }));
        });
    }
